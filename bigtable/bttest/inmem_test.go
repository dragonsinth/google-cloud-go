...
func TestConcurrentMutationsReadModifyAndGC(t *testing.T) {
	...
	case <-time.After(3 * time.Second):
		t.Error("Concurrent mutations and GCs haven't completed after 3s")
	}
}
...
func TestDropRowRange(t *testing.T) {
	...
	numRows := func() int {
		...
		for _, rsp := range mock.responses {
			for _, c := range rsp.Chunks {
				keys[string(c.RowKey)] = true
			}
		}
		return len(keys)
	}
...
func TestFilterRowWithUnicodeColumnQualifier(t *testing.T) {
	...
	for _, test := range []struct {
		...
		got, _ := filterRow(&btpb.RowFilter{Filter: &btpb.RowFilter_ColumnQualifierRegexFilter{[]byte(test.filter)}}, makeRow())
		if got != test.want {
			t.Errorf("%v: got %t, want %t", test.filter, got, test.want)
		}
	}
